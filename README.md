...
- **Body turns child‑like (big head, short legs) in full‑body shots** — MSR locks the face, not the body. Set `height` / `build` (they now describe proportions), keep the negative prompt's body terms, and for the strongest fix put a full‑body photo of the singer in MSR REF 2. / 全身ショットで子供体型になる→ MSR は顔だけ固定。`height`/`build` を指定（プロポーションで記述される）、ネガティブの体型項目を残す、確実なのは MSR REF 2 に全身写真。

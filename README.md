...
| **LTX Chain: Auto Scenes** | Florence‑2 caption → full singing prompt; ethnicity/age/singers/performance/extra cameras/microphone/gender/emotion/motion/height/build/camera/outfit, plus **look / lighting / setting / weather** presets composed with the free `style` text (`weather_when` puts the weather on all scenes or only some). / 画像→プロンプト自動生成（服装補足・画調/照明/舞台/天候プリセット付き） |

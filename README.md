...
- **Output size / 書き出しサイズ** — pick a size from the **`resolution`** dropdown (LTX‑2.5‑safe presets, multiples of 64: portrait `576×1024`, square `768×768`, landscape `1024×576`, …). `auto` keeps the **input image's aspect ratio** and uses the `generation_width × height` pixel area (this is why the Paris sample came out `576×720`). Bigger = more VRAM; on 12 GB keep the area around ~600k px. **`576×1024` is the size to use when the shot is wide** — see the distant-faces tip below. / `resolution` から選ぶ（64 の倍数の安全サイズ）。`auto` は画像の縦横比のまま `generation_width×height` の面積で決定。大きいほど VRAM を使うので 12GB は ~600k px 目安。**引いた画が多いなら `576×1024`**（下の「遠目の顔」参照）。
...
- **Faces go soft or mis‑shapen in wide / full‑body shots — raise `resolution` to `576×1024`** — the shape of a face is decided in **stage 1, which runs at half the output size**, and the LTX VAE packs 32×32 pixels into one latent cell. Measured on a 512×896 run: a close‑up face is ~300 px tall (≈4.7 latent cells at stage 1) and renders cleanly; a knee‑up shot ~150 px (≈2.3 cells) is already softer; a full‑body shot is ~50–70 px (**≈1 cell**), which is not enough to place eyes, nose and mouth — stage 2 then invents a plausible face, and it differs from shot to shot. `576×1024` takes the stage‑1 latent grid from 8×14 to 9×16 cells (**+29 % cells**, 458k → 590k px) and is the single setting that helps distant faces; it costs ~1.3× VRAM and time and still fits 12 GB. It is an improvement, not a cure: at true full‑body framing the face is still about one latent cell, so **framing tighter (waist‑up) remains the bigger lever**, and a whole‑video face pass (`ReActor-post-process-video.json`) is the fallback. The `(test …)` presets are far worse again — never judge faces on them. / 引き・全身で顔が崩れる→ `resolution` を `576×1024` に。顔の形を決める Stage 1 は出力の半分の解像度で走り、VAE は 32×32 px を 1 latent セルに圧縮する。512×896 実測で、アップの顔は約 300 px（Stage 1 で約 4.7 セル）＝破綻なし、膝上は約 150 px（約 2.3 セル）＝やや甘い、全身は約 50〜70 px（**約 1 セル**）＝目鼻口を置く情報が無く、Stage 2 が毎回それらしい顔を描き足すので形が変わる。`576×1024` は Stage 1 の latent 格子を 8×14 → 9×16（**セル数 +29%**、458k → 590k px）に増やし、遠目の顔に効く唯一の設定。VRAM と時間は約 1.3 倍で 12GB に収まる。ただし完全な解決ではなく、真の全身では依然として約 1 セルなので、**寄りで撮る（腰から上）ほうが効果は大きい**。最後の手段は動画全体への顔パス（`ReActor-post-process-video.json`）。`(test …)` はさらに悪いので顔の判断に使わないこと。
...
- **Nobody sings anywhere — every clip is `intro` and the log says `vocal ratio 0.00`** — check **`vocal_threshold_db`** on Scene Prompt / Auto Scenes: it must be **negative**, and `-40` is the default. The value is dBFS and the test is `window RMS > threshold`, so with a normalised waveform the level is always ≤ 0 dBFS; a threshold of `0` (or anything above it) matches no window at all and the ratio is 0.00 for the whole song, which is below `min_vocal_ratio` everywhere. `force_singing_clips` hides it clip by clip but the detection stays dead. A logged `vocal ratio -1.00` is a *different* fault: it means the `vocals` input is not connected, so wire MelBandRoFormer's **first** output (`vocals`, not `instruments`) into it. / どのクリップも歌わず、ログが `vocal ratio 0.00` → Scene Prompt / Auto Scenes の **`vocal_threshold_db`** を確認。**負の値**である必要があり、既定は `-40`。単位は dBFS で判定は「窓の RMS > 閾値」。正規化された音声は常に 0 dBFS 以下なので、閾値が `0` だとどの窓も条件を満たさず、全曲で ratio 0.00 ＝ `min_vocal_ratio` を必ず下回る。`force_singing_clips` で個別に押し切ることはできるが検出自体は死んだまま。ログが `vocal ratio -1.00` なら別の原因で、`vocals` が未接続（MelBandRoFormer の**1番目**の出力 `vocals` をつなぐ。`instruments` ではない）。

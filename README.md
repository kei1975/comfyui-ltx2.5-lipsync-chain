...
Key State parameters / 主な State 設定: `chunk_seconds` (per‑clip length 1クリップ秒数), `length_mode` (`all`/`seconds`), `resolution` (presets; the `(test …)` sizes are ~480p previews for motion/camera checks and pop up a notice — not for judging faces), `clips_per_scene`, `scene_cuts`, `scene_switching` (one‑button ON/OFF for scene changes; OFF keeps the cuts but renders one angle), `msr_clips` (`stage2_all`; use `all` for body‑proportion drift, then give stage 1 the full‑body reference only), `scene_crossfade`, `overlap_frames`, `handoff_color_match`, `seed`, `redo_session` / `redo_clips`.
`resolution`（`(test …)` は動き・カメラ確認用の 480p 相当。選ぶと注意ダイアログ）、`scene_switching`（シーン切替のワンボタン。OFF でも cuts は保持）、`msr_clips`（体型ドリフトには `all` ＋ Stage 1 は全身参照のみ）。

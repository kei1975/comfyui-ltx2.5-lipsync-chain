...
class LTXChainAutoScenes:
    ...
    SETTINGS = {
        ...
        # the MSR guide's `background` slot (REF 5): the model sees the place itself; the Florence-2
        # caption of that image (background_caption input) is appended when connected
        "background reference (REF 5)": "the location shown in the background reference image",
    }
    ...
    # pace of a moving motion. The presets say "slowly" / "steady pace" themselves, so when a speed
    # is chosen those words are stripped first and the pace sentence is appended instead.
    MOTION_SPEEDS = {
        "auto (as written)": "",
        "very slow": "at a very slow, unhurried pace, each step deliberate and drawn out",
        "slow": "at a slow, relaxed pace",
        "normal": "at a normal, natural walking pace",
        "brisk": "at a brisk, purposeful pace with quick steps",
        "fast": "at a fast pace, hurrying, quick energetic steps",
        "on the beat": "with every step landing exactly on the beat of the music, the pace locked to the song's tempo",
        "half-time": "with a step on every other beat, a slow groove at half the song's tempo",
        "double-time": "with two quick steps per beat, the pace driven by the song's rhythm",
    }
    SPEED_WORDS = (" slowly and steadily", " slowly", " steadily", " at a steady pace", " lightly", " relaxed steady", " relaxed")
        ...
        return {
            "required": {
                ...
                "setting": (list(cls.SETTINGS.keys()), {"default": "auto (from photo)",
                            "tooltip": "舞台（背景）のプリセット。auto=写真の背景のまま。それ以外を選ぶと「写真の背景の代わりにこの舞台」として全シーンに入ります（recording studio / plain backdrop / white cyclorama / concert stage / club stage / theater / rooftop / street at night / neon alley / bedroom / loft / rainy window / bar / warehouse / forest / beach）。background reference (REF 5) = MSR の background スロットに入れた画像の場所（REF 5 と「背景を説明させる」Florence ノードのバイパスを解除し、Auto Scenes の background_image / background_caption につなぐ）"}),
            ...
            "optional": {
                ...
                "background_image": ("IMAGE", {"tooltip": "任意。REF 5（MSR の background スロット）と同じ画像。キャッシュのキーに使うだけ（画像を替えたらプロンプトを作り直す）"}),
                "background_caption": ("STRING", {"forceInput": True, "lazy": True,
                                                  "tooltip": "任意。REF 5 を Florence-2 Run（more_detailed_caption）で説明させた caption。setting = background reference (REF 5) のときだけ評価され、舞台の説明としてプロンプトに入ります"}),
            },
        }
    ...
    def _build(self, image, num_scenes, style, caption, num_singers=1, ethnicity="", performance="restrained", age="", extra_cameras="", microphone="auto", gender="auto", emotion="none", emotion_custom="", motion="none", motion_custom="", height="none", build="none", camera="auto", camera_custom="", outfit="", look="custom (style text only)", lighting="auto (from photo)", setting="auto (from photo)", weather="none", weather_when="all scenes", vfx="none", vfx_when="random (about a third)", vfx_custom="", framing_limit="wide", motion_speed="auto (as written)", season="auto (from photo)", framing_min="close-up", background_people="", background_image=None, background_caption=""):
        ...
        if pace and mot_sing and motion not in ("none", "standing still", "sitting"):
            for wd in self.SPEED_WORDS:
                mot_sing = mot_sing.replace(wd, "")
                mot_listen = mot_listen.replace(wd, "")
            mot_sing = mot_sing.replace("  ", " ").strip() + f", {pace}"
            mot_listen = mot_listen.replace("  ", " ").strip() + f", {pace}"
            if cam_note:  # the follow camera keeps whatever pace the singer has
                cam_note = cam_note.replace("at the same pace", "at exactly the singer's pace").replace("at the same speed", "at exactly the singer's speed")
